...
/**
 * @description 简历具体几页
 * @returns {number} pageNum
 */
export function getResumeElementHeight(): number {
  let resumeWidth = document.getElementById('visPdf')?.offsetWidth;
  let resumeHeight = document.getElementById('visPdf')?.offsetHeight;
  if (resumeWidth) {
    let height = (resumeWidth / A4Width) * A4Height;
    if (resumeHeight) {
      return Math.ceil(resumeHeight / height);
    } else {
      return 1;
    }
  } else {
